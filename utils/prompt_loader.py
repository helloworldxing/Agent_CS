...
def load_system_prompts():
    try:
        ...
        logging.error(f"[load_system_prompts]在yaml配置中没有main_prompt_path配置项")
        raise e

    try:
        return open(system_prompts_path, 'r', encoding='utf-8').read()
    except Exception as e:
        logging.error(f"[load_system_prompts]解析系统提示词出错,{str(e)}")
        raise e
...
def load_rag_prompts():
    try:
        ...
        logging.error(f"[load_rag_prompts]在yaml配置中没有main_prompt_path配置项")
        raise e

    try:
        return open(rag_prompts_path, 'r', encoding='utf-8').read()
    except Exception as e:
        logging.error(f"[load_rag_prompts]解析系统提示词出错,{str(e)}")
        raise e
...
def load_report_prompts():
    try:
        ...
        logging.error(f"[load_report_prompts]在yaml配置中没有report_prompt_path配置项")
        raise e

    try:
        return open(report_prompts_path, 'r', encoding='utf-8').read()
    except Exception as e:
        logging.error(f"[load_report_prompts]解析系统提示词出错,{str(e)}")
        raise e
